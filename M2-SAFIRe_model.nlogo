...
to setup
  ca
  set run-seed new-seed random-seed run-seed ;; attention doit être commenté pour aller dans openMole
  set year 0
  set day-of-year 0
  ...
  set nb-champs-visités _nb-fields / nb-surveillants
...
to surveillance-representant
  ...
  ;poser l'hypothèse - plus il y a de surveillants plus la proba de voir des coupeurs est forte car ils visitent moins de champs.
...
to coupe-pousse
  ...
  [
    ...
    set nbTreekilled_cutter nb-coupe
...
to update-variables
  ...
  set %-under-tree (total-under-tree-area / (count patches * patch-area)) * 100
  set nb-arbres count trees
  set age-moy-arb  [age-tree] of trees
  set pouss count pousses
  set pouss-prot count pousses with [signalé = TRUE]
  set pouss-inter-prot count pousses with [age > age-p-interm]
  set MoyN-interet-RNA mean [interet-RNA] of agriculteurs
...
<experiments>
  ...
  <experiment name="calibration-surveillance-déléguée" repetitions="5" runMetricsEveryStep="true">
    ...
    <steppedValueSet variable="malus-nb-chps" first="0.1" step="0.1" last="0.6"/>
    <enumeratedValueSet variable="tps-au-champ">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="participants">
